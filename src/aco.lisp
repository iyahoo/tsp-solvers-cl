...
(defun ACO (tau beta rho m q init-pos last-generation &optional (file-out-p nil) (file-name ""))
  ...
  (let ((final-ants)
    ...
    (with-open-file (output (merge-pathnames (append "results/" file-name))
      ...
      (dotimes (g last-generation (list *tau-matrix* final-ants shortest-path))
        (let ((ants (make-ants *init-pos*)))
          ...
          (map (lambda (ant)
                 (when (< (length-of-route ant) (second shortest-path))
                   (setf shortest-path (list (ant-route ant) (length-of-route ant)))))
               ants))))))
