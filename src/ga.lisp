...
(defun GA (m crossover-point mutation-rate last-generation &optional file-out-p (file-name ""))
  ...
  (let ((agents (make-agents)))
    ...
    (let ((final-agents (iota 10))
          (shortest-path '(nil 1000)))
      (with-open-file (output (merge-pathnames (append "results/" file-name))
                              :direction :output :if-does-not-exist :create :if-exists :supersede)
        (dotimes (g last-generation (list final-agents shortest-path))
          ...
          (when file-out-p
            (map (lambda (agent)
                 ...
                 agents))
          (map (lambda (agent)
                 (when (< (length-of-route agent) (second shortest-path))
                   (setf shortest-path (list (ordered->normal (agent-route agent)) (length-of-route agent)))))
               agents)
